...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='landing', rocket_type='falcon',
        ...
        self.env_params = {'rho': self.m_simu / (125/(self.g/2.0))**0.5, 'q': 0, 'H': self.H, 'W': self.H/10, 'I': self.I, 'count': 0, 'm': self.m_simu, 'g': self.g, 'wind_strength': self.wind_strength}
    ...
    def reset(self, state_dict=None, seed=None):
        ...
        if state_dict and seed is None:
            ...
            'phi': 0, 'f': 0,
            't': 0, 'a_': 0,
            'delta_L': np.deg2rad(90), 'delta_R': np.deg2rad(90),
            'q': 0, 'vphi': 0,'vdelta': 0
            }
    ...
    def create_action_table(self):
        ...
        f1 = 0.2 * self.env_params['m_simu'] * self.env_params['g']
        f2 = 1.0 * self.env_params['m_simu'] * self.env_params['g']
        f3 = 2.0 * self.env_params['m_simu'] * self.env_params['g']
        vphi1 = 30 / 180 * np.pi             # 舵面角速度
        vphi2 = -30 / 180 * np.pi
        vphi3 = 0

        action_table = [[f1, vphi1], [f1, vphi2], [f1, vphi3]
                        , [f2, vphi1], [f2, vphi2], [f2, vphi3]
                        , [f3, vphi1], [f3, vphi2], [f3, vphi3]]
        return action_table
    ...
    def calculate_reward(self, state):
        ...
        dist_reward = 1.0 * np.exp(-2.0 * dist_norm)
        ...
        phi_penalty = -0.02 * (abs(state['vphi']) / (90*np.pi/180))

        velocity_penalty = -0.1 * abs(state['vx']) / 50.0

        reward = dist_reward + theta_penalty + fuel_penalty + phi_penalty + velocity_penalty
    ...
    def step(self, action=None):
        ...
        if self.already_landing:
            ...
            phi, f = 0, 0
            delta_L, delta_R = 0, 0
            action, vphi, vdelta_L, vdelta_R, self.env_params['q'] = 0, 0, 0, 0, 0
        ...
        self.state = {
            ...
            'q': self.env_params['q'], 'vphi': vphi, 'vdelta': vdelta_L
        }
    ...
    def flatten(self, state):
        x = [state['x']/150, state['y']/510, state['vx']/50, state['vy']/100,
             ...
             state['f']/(2*self.g*self.m_simu), state['vphi']/np.deg2rad(30), state['vdelta']/np.deg2rad(30), state['q']/self.q_ref]
    ...
    def create_polygons(self):
        ...
        if f == 0.2 * self.env_params['g'] * self.env_params['m_simu']:
        ...
        elif f == 1.0 * self.env_params['g'] * self.env_params['m_simu']:
            pts1 = utils.create_rectangle_poly(center=(2 * dl * s, -H / 2 - 2 * dl * c), w=dl, h=dl)
            pts2 = utils.create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
        ...
        elif f == 2.0 * self.env_params['g'] * self.env_params['m_simu']:
            pts1 = utils.create_rectangle_poly(center=(2 * dl * s, -H / 2 - 2 * dl * c), w=dl, h=dl)
            pts2 = utils.create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
            pts3 = utils.create_rectangle_poly(center=(8 * dl * s, -H / 2 - 8 * dl * c), w=2 * dl, h=2 * dl)
