...
if __name__ == '__main__':
    ...
    env = ConditionalFenv()
    ckpt_folder = os.path.join('./', 'co_landing' + '_ckpt')
    ...
    for episode_id in range(last_episode_id, max_m_episode):
        ...
        env.env.settling_time, env.env.fuel, env.env.landing_x = 0, env.env.max_fuel, 0
        time = []
        for step_id in range(max_steps):
            ...
            effort += np.abs(env.env.state['vphi']) + np.abs(env.env.state['vdelta'])

            is_stable = np.abs(env.env.state['theta']) < (5/180*np.pi) and np.abs(env.env.state['x']) < 15
        ...
        env.env.settling_time = current_stable_steps * env.env.dt 
        
        if current_stable_steps > 0:
            env.env.adjust_time = final_adjust_step * env.env.dt
        else:
            env.env.adjust_time = max_steps * env.env.dt  

        REWARDS.append(np.sum(rewards))
        FUEL.append(env.env.fuel)
        LANDING_X.append(env.env.landing_x)
        SETTLING_TIME.append(env.env.settling_time)
        ADJUST_TIME.append(env.env.adjust_time)
        EFFORT.append(effort)

        print('episode id: %d, episode reward: %.3f, seed_id: %d'
              % (episode_id, np.sum(rewards), seed_id))
        if np.sum(rewards) < 0:
            print('Wind Strength: %.3f' % env.env.env_params['wind_strength'])
