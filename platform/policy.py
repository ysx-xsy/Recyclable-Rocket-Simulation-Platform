...
class Residual(nn.Module):
    ...
    def _unscale_actions(self, scaled_actions):
        """
        将动作从指定范围映射回[-1,1]范围（用于计算概率时）
        scaled_actions: shape [..., action_dim]
        """
        actions = torch.zeros_like(scaled_actions)
        for i, (min_val, max_val) in enumerate(self.action_ranges):
            # 从[min_val, max_val]映射回[-1,1]
            actions[..., i] = (scaled_actions[..., i] - min_val) * 2.0 / (max_val - min_val) - 1.0
        return actions
