...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='landing', rocket_type='falcon',
        ...
        self.action_total = [0, 0, 0]  # [f, vphi, vdelta]
    ...
    def create_action_table(self, a = 0, b = 0):
        f = a
        vphi = b
        vdel_1 = 30 / 180 * np.pi             # 舵面角速度
        vdel_2 = -30 / 180 * np.pi
        vdel_3 = 0

        action_table = [[f, vphi, vdel_1], [f, vphi, vdel_2], [f, vphi, vdel_3]]
        return action_table
    ...
    def step(self, action=None):
        ...
        f, vphi, vdelta_L = self.action_table(action)
        vdelta_R = vdelta_L # 设定两个舵面动作相同
        ...
        delta_L = np.clip(delta_L, -np.deg2rad(30), np.deg2rad(30))
        delta_R = np.clip(delta_R, -np.deg2rad(30), np.deg2rad(30))
        phi = np.clip(phi, -np.deg2rad(20), np.deg2rad(20))
