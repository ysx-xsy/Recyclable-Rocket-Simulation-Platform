    ...
    def __init__(self):
        ...
        self.SCENARIOS = {
            ...
            6: {"name": "Large angle tilt", 
                "x": (-90), "y": (480, 520), "vx": (-5, 5), "vy": (-130, -120), "theta": (-60), "wind": (-5, 5)},
            7: {"name": "Extreme conditions", 
                "x": (-200, 200), "y": (400, 450), "vx": (-15, 15), "vy": (-140, -130), "theta": (-90, 90), "wind": ((-15, -10),(10, 15))},
        }
...
class Rocket(object):
    ...
    def __init__(self, max_steps, task='landing', rocket_type='falcon',
        ...
        self.env_params = {'rho': self.m_simu / (125/(self.g/2.0))**0.5, 'q': 0, 'fin_area': self.fin_area, 'H': self.H, 'W': self.H/10, 'I': self.I, 'count': 0, 'm': self.m_simu, 'g': self.g}
    ...
    def reset(self, state_dict=None, seed=None, state_range=None):
        
        if state_dict is not None and seed is None:
            x, y, vx, vy, theta = state_dict['off_centering'], state_dict['initial_height'], state_dict['horizontal_velocity'], state_dict['vertical_velocity'], state_dict['pitch_angle']
            self.base_wind = state_dict['wind_strength']
            self.state = {
            'x': x, 'y': y, 'vx': vx, 'vy': vy,
            ...
            if state_range is not None and seed is None:
                scenario = state_range
    ...
    def step(self, action=None):
        ...
        f, vphi, vdelta_L, vdelta_R, _, _ = self.pid_controller.get_action(self.state, self.env_params, self.wind_strength)
        ...
        alpha_total_R = alpha_base + delta_R
        self.aoa.append(alpha_total_L*180/np.pi)
        ...
        ft = -f * np.sin(phi) + ft_fin_total
        fr =  f * np.cos(phi) + fr_fin_total
        ...
        dm = f/self.m_simu*self.m_true*4*self.dt/(self.Isp*self.g)
    ...
    def create_polygons(self):
        ...
        if f > 0 and f <= 0.5 * self.g * self.m_simu:
        ...
        elif f > 0.5 * self.g * self.m_simu and f < 1.5 * self.g * self.m_simu:
            pts1 = utils.create_rectangle_poly(center=(2 * dl * s, -H / 2 - 2 * dl * c), w=dl, h=dl)
            pts2 = utils.create_rectangle_poly(center=(5 * dl * s, -H / 2 - 5 * dl * c), w=1.5 * dl, h=1.5 * dl)
    ...
    def draw_text(self, canvas, color=(255, 255, 0)):
        ...
        pt = (10, 180)
        text = "f: %.2f, phi: %.2f" % (self.state['f'], self.state['phi'])
        put_text(canvas, text, pt)
